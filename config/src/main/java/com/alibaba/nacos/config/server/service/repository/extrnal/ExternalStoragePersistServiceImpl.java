...
public class ExternalStoragePersistServiceImpl implements PersistService {
    ...
    public void removeConfigHistory(final Timestamp startTime, final int limitSize) {
        ...
        try {
            if("gbasedbt".equalsIgnoreCase(this.dbType)){
                sql =  "delete from  his_config_info where nid in( select * from ((select first ? nid from his_config_info where gmt_modified<?) as t))";
                helper.updateLimit(sql, new Object[]{limitSize, startTime});
            }else {
